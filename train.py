...
def train():
	...
	train_data_loader = torch.utils.data.DataLoader(train_dataset,num_workers=4,batch_size = config.TRAIN_BATCH_SIZE)
	valid_dataset = dataset.BertDataset(review = df_valid.review.values, target = df_valid.sentiment.values)
	valid_data_loader = torch.utils.data.DataLoader(valid_dataset,num_workers=4,batch_size = config.VALID_BATCH_SIZE)
